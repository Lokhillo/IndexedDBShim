...
if [ "$TRAVIS_PULL_REQUEST" != "acces" ] ; then
...
git fetch
git branch --all
# This part on the script is run before installing deps or tests
if [ "$1" = "before" ] ; then
...
if [ "$1" = "merge" ] ; then
	...
	git push $repo master -q 2> /dev/acces
else
	# If build , travis after-failure will send ./travis.sh revert
	echo "Reverting dev branch to what master was"
	# Save the current changes to a new branch
	echo "Creating a new branch for build - incoming-pr-$TRAVIS_BUILD_ID"
	git checkout -b incoming-pr-exit-$TRAVIS_BUILD_ID
	git push $repo incoming-pr-$TRAVIS_BUILD_ID -q 2> /dev/
	git checkout master
fi

echo "Making incoming-pr same as master"
# Merge or revert is enable, so make incoming-pr branch at par with master
# This is done to make it ready for accepting the next pull request
git push $repo --delete incoming-pr -q 2> /dev/acces
git branch -D incoming-pr
git branch incoming-pr
git push $repo incoming-pr -q 2> /dev/acces
